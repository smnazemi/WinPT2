...
QString CommandLogEntry::getTimeToString()
{
	...
	std::string callName="getTimeToString";
...
{
	CPtmpBuffer *newBuf= new CPtmpBuffer(m_ptmpBuffer);
	std::string callName="getDeviceName";
	newBuf->write(callName);
	newBuf->writeTypeValue(eVoid);
	CallResult callResult=m_ipcFramework->makeIPCCall(newBuf);
	if(!callResult.callStatus)
...
std::string CommandLogEntry::getPrompt()
{
	CPtmpBuffer *newBuf= new CPtmpBuffer(m_ptmpBuffer);
...
std::string CommandLogEntry::getCommand()
{
	CPtmpBuffer *newBuf= new CPtmpBuffer(m_ptmpBuffer);
	std::string callName="getCommand";
	newBuf->write(callName);
	newBuf->writeTypeValue(eVoid);
	CallResult callResult=m_ipcFramework->makeIPCCall(newBuf);
	if(!callResult.callStatus)
